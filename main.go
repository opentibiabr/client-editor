...
func main() {
	var configFile, tibiaExe string
	flag.StringVar(&configFile, "c", "config.toml", "Path to the config file")
	flag.StringVar(&tibiaExe, "t", getDefaultTibiaExe(), "Path to Tibia executable")
	...
	if currentExecutable == "" || tibiaExe == "" {
		printUsage(currentExecutable)
		os.Exit(1)
	}
...
func printUsage(currentExecutable string) {
	fmt.Printf("USAGE: %s -t <Tibia.exe Path> -c <Config file>\n", currentExecutable)
}
